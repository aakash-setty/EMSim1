...
# The voice order terminology. Generated by catalog/build_voice_aliases.py from the
# phrases authored in that script; keyed by catalog id, resolved to the case's ids by
# the engine at bind. Optional: a checkout without it builds a simulator whose
# microphone accepts only the catalog's display names.
_voice_path = catalog_path("voice-aliases.json")
voice = json.load(open(_voice_path)) if os.path.exists(_voice_path) else {}
...
def main():
    ...
    bundle = ("/*__ENGINE_START__*/\n" + open(os.path.join(HERE, "engine.js")).read() +
              ...
              # voice.js declares `const VOICE` and ui.js calls VOICE.build from bindCase,
              # which boot() reaches at the end of ui.js, so it MUST come before ui.js for
              # the same reason semantic.js must. Its own top level touches nothing from
              # ui.js; the calls it makes into ui.js (log, render, inCase) run from
              # handlers, long after both have loaded. The parser is fenced so the test
              # harness can evaluate it without the page.
              "\n/*__VOICE_START__*/\n" + open(os.path.join(HERE, "voice.js")).read() +
              "\n/*__VOICE_END__*/\n" +
    ...
    shell = shell.replace("__VOICE_JSON__", jsafe(voice))
