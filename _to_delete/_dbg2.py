...
CANON = [
    ...
    [r"500 (saline|lr|bolus|d5)", r"halfliter \1"], [r"(saline|lr|bolus|d5) 500", r"\1 halfliter"],
    [r"250 (saline|lr|bolus|d5)", r"quarterliter \1"], [r"(saline|lr|bolus|d5) 250", r"\1 quarterliter"],
    ...
    [r"(abdomen and pelvis|abdomen pelvis|a p|a and p|abd pelvis|abd|belly|abdominal|abdomen|ap)", "abdomen"],
    ...
    [r"(dissection protocol|dissection|aortogram|aortagram|chest abdomen pelvis|chest abdomen and pelvis|cap|c a p)", "aorta"],
...
def normalise(text):
    ...
    out = []
    for w in t.split():
        if w in FILLERS or w in UNITS: continue
...
alias("xr_chest", "chest xr", "xr chest", "cxr", "portable chest", "portable chest xr", "chest", "pa and lateral",
...
alias("magnesium_sulfate", "magnesium sulfate", "mag sulfate", "magnesium sulphate", "mag sulphate", "mgso4", "mg so4", "iv magnesium", "iv mag", "magnesium iv",
...
A["magnesium_sulfate_bolus"] = [p for p in A["magnesium_sulfate"] if "bolus" in p or "push" in p]
...
for p in ["whole blood", "low titer o whole blood", "ltowb", "low titer whole blood", "fresh whole blood", "cell saver", "autotransfusion", "auto transfusion", "rapid infuser", "level one", "level 1", "belmont", "rapid transfuser", "blood warmer", "fluid warmer"]:
...
print("DBG2", [p for p in aliases["basic_chemistry_chem_7"] if "renal" in p], normlist(["renal function"]))
def normmap(d):
    out = {}
    for p, v in d.items():
        if DROP_RE.search(p.strip().lower()): continue
...
print("DBG3", [p for p in out["aliases"]["basic_chemistry_chem_7"] if "renal" in p]); json.dump(out, open(OUT, "w"), indent=1, ensure_ascii=False)
