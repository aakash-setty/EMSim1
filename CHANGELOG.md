...
Investigations, Interventions and Stabilization, click it again, and a list drops down of
what was understood: one row per order with a red cross beside it, a Confirm button pinned
under the list however long it scrolls, and Confirm sends the rows through the same log path
as Submit Order, one entry per order at the same instant, so prerequisites and harmful tags
evaluate exactly as they do from the tabs. The list also takes typed text, in the same words
and through the same parser, which is what a browser without speech recognition gets.

**The recogniser is the browser's** (Web Speech API): Chrome, Edge and Safari; not Firefox.
That is a third party, and a fourth outbound request: the audio goes to the browser vendor's
speech service and the README says so beside the other three. A session is restarted through
the silences the browser ends it on, is capped at two minutes, stops when the case pauses, and
the permission prompt no longer pauses the case by taking the window's focus, which it did on
the first try. Every failure ends in the typed box rather than a dead button.

**The terminology is a table, not the parser.** `catalog/voice-aliases.json`, generated by
`catalog/build_voice_aliases.py`, keyed by catalog id and resolved to the case's ids at bind,
holds the ways residents say each entry (EKG, ECG, electrocardiogram, twelve lead; VBG, venous
gas, venous resuscitation panel; heart, cardiac and POCUS of the heart) plus three other kinds
of row. An **expansion** is one phrase that is several orders: CMP is the Chem 7 plus the
hepatic panel, MTP is red cells, plasma and platelets, DuoNeb is albuterol and ipratropium,
"two large bore IVs" is the first and the second IV. An **ambiguity** is a phrase that could
mean two entries and is offered as a choice in the row rather than resolved by frequency: a
bare "epi" (bolus, drip or IM), "magnesium" (the level or the drug), "ultrasound" with no view,
"central line" (triple lumen or cordis), "saline" with no volume. **Unavailable** is a phrase
residents say that the catalog does not carry, refused with a hint rather than substituted:
calcium gluconate is not swapped for calcium chloride, Unasyn is not swapped for ampicillin,
and "sepsis workup" is refused outright, because a parser that expanded a diagnosis into its
order set would be answering the case for the resident. The two author-stated exceptions are
recorded beside the rule: a bare "calcium" is the level, and CMP is the two panels.

**Doses, units, rates and reasons are stripped, not matched.** "Give one milligram of
epinephrine IV push for the arrest" reaches the table as "epi bolus"; a number survives only
where it discriminates (500 against a litre, 15 litres on a mask, twelve lead). A reason that
follows an order ("troponin for the chest pain") is dropped silently; a word that matches
nothing is shown as not understood; one misheard letter in a word of six or more is repaired
only when exactly one phrase is that close, and the row says what was heard and what it was
taken as. Nothing outside the three orderable tabs is ever an order: exams and consults are
not spoken.

**Two normalisers, one test.** The table is written already normalised by the Python pipeline
that generated it, and `voice.js` carries the same pipeline for speech. They are the same
pipeline only as long as someone checks, so `engine-tests.js` normalises every phrase in the
shipped table with the JS side and requires each back unchanged, and runs the author's
examples and every display name through the parser. The pipeline runs to a fixed point,
because a filler dropped between two words can produce a phrase a rule names ("oxygen by
mask") and one rule's output is another's input ("kidney function" is renal function is a
Chem 7); the first version ran once and the test caught eight phrases it had left half done.

**What it does not do.** It does not order across the tabs it is not allowed to, it does not
understand a dose as a dose, and it does not run offline: the parser does, the recogniser
does not. An in-browser model was considered and set aside for the same reason the interview
matcher's is optional, see `docs/decisions/voice-orders.md`.
